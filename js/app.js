...
var ISO3_TO_2 = {
    DEU:'DE', FIN:'FI', USA:'US', AUS:'AU', CAN:'CA',
    NLD:'NL', SGP:'SG', GBR:'GB', IND:'IN', JPN:'JP'
};
...
var COLORS = { hetzner:'#7c3aed', digitalocean:'#2563eb', linode:'#0891b2', multi:'#059669' };

Promise.all([
    fetch('js/pricing.json').then(function(r) { return r.json(); }),
    fetch(GEOJSON_URL).then(function(r) { return r.json(); })
]).then(function(res) {
    pricingData = res[0];
    buildCountryIndex();
    initMap(res[1]);
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        for (var j = 0; j < prov.regions.length; j++) {
            ...
            for (var k = 0; k < countryIndex[c].length; k++) {
                if (countryIndex[c][k].provider === key) { dup = true; break; }
            }
            if (!dup) {
                ...
                if (r.planCode) {
                    for (var m = 0; m < prov.plans.length; m++) {
                        if (prov.plans[m].code === r.planCode) { regionPlan = prov.plans[m]; break; }
                    }
                }
                countryIndex[c].push({
                    provider: key, displayName: prov.displayName,
                    region: r, plan: regionPlan, billingAlerts: key !== 'linode'
                });
            }
        }
    }
...
function initMap(geo) {
    leafletMap = L.map('map', {
        center: [20, 0], zoom: 2, minZoom: 2, maxZoom: 5,
        worldCopyJump: false,
    ...
    geoLayer = L.geoJSON(geo, {
        style: styleCountry, onEachFeature: bindFeatureEvents, noWrap: true
    }).addTo(leafletMap);
...
function addSmallCountryMarkers() {
    ...
    for (var i = 0; i < tinyCountries.length; i++) {
        ...
        var marker = L.circleMarker([tc.lat, tc.lng], {
            radius: 8, fillColor: color, fillOpacity: 0.8, color: '#fff', weight: 2
        }).addTo(leafletMap);
                ...
                click: function() {
                    if (opts.length === 1) {
                        ...
                        for (var j = 0; j < opts.length; j++) {
                            var o = opts[j];
                            html += '<button class="picker-option" data-provider="' + o.provider + '">'
                                + '<div class="picker-option-header"><strong>' + o.displayName + '</strong></div>'
                                + '<div class="picker-option-specs">' + o.plan.vcpu + ' vCPU / ' + o.plan.ram + ' / ' + o.plan.disk + '</div>'
                                + '<div class="picker-option-price">$' + o.plan.price.toFixed(2) + '/mo'
                                + ' <span class="picker-region">&middot; ' + o.region.name + '</span></div>'
                                + '</button>';
                        }
                            ...
                            (function(btn, allOpts) {
                                btn.addEventListener('click', function() {
                                    ...
                                    for (var m = 0; m < allOpts.length; m++) {
                                        if (allOpts[m].provider === btn.dataset.provider) { chosen = allOpts[m]; break; }
                                    }
...
function showTooltipForMarker(e, name, opts) {
    ...
    for (var i = 0; i < opts.length; i++) {
        ...
        rows += '<div class="tt-provider"><strong>' + o.displayName + '</strong><br>'
            + '<span class="tt-spec">' + o.plan.vcpu + ' vCPU / ' + o.plan.ram + ' / ' + o.plan.disk + '</span><br>'
            + '<span class="tt-price">$' + o.plan.price.toFixed(2) + '/mo</span>'
            + '<span class="tt-region"> &middot; ' + o.region.name + '</span></div>';
    }
...
function styleCountry(feature) {
    ...
    if (!opts || opts.length === 0) return { fillColor:'#cbd5e1', fillOpacity:0.4, color:'#fff', weight:0.5 };
...
function bindFeatureEvents(feature, layer) {
    ...
    layer.on({
        ...
        mouseout: function(e) {
            if (ISO3_TO_2[feature.id] !== state.selectedCountry) geoLayer.resetStyle(e.target);
            hideTooltip();
        },
        click: function() { selectCountry(feature, opts); }
    });
}
...
function showTooltip(e, feature, opts) {
    ...
    tooltipEl.innerHTML = '<div class="tt-country">' + feature.properties.name + '</div>' + rows;
    tooltipEl.style.display = 'block';
    positionTooltip(e.originalEvent);
}
...
function positionTooltip(ev) {
    ...
    var x = ev.clientX - rect.left + 14, y = ev.clientY - rect.top - 10;
    var tw = tooltipEl.offsetWidth || 230, th = tooltipEl.offsetHeight || 80;
            ...
            (function(btn, allOpts) {
                btn.addEventListener('click', function() {
                    ...
                    for (var k = 0; k < allOpts.length; k++) {
                        if (allOpts[k].provider === btn.dataset.provider) { chosen = allOpts[k]; break; }
                    }
                    commitSelection(iso2, chosen);
                    providerPicker.style.display = 'none';
                });
...
function commitSelection(iso2, option) {
    ...
    geoLayer.eachLayer(function(l) {
        ...
        if (!o) return;
        l.setStyle(ISO3_TO_2[l.feature.id] === iso2
            ? { fillOpacity:1, weight:2.5, color:'#fff' }
            : { fillOpacity:0.2 });
    });

    selectionText.textContent = option.displayName + ' \u00b7 ' + option.region.name
        + ' \u00b7 ' + option.plan.name + ' \u00b7 $' + option.plan.price.toFixed(2) + '/mo';
...
function collapseDownstreamCards() {
    ...
    if (el('tailscaleAccountCheck')) el('tailscaleAccountCheck').style.display = 'block';
...
clearSelectionBtn.addEventListener('click', function() {
    state.selectedCountry = state.selectedProvider = state.selectedPlan = state.selectedRegion = null;
    geoLayer.eachLayer(function(l) { geoLayer.resetStyle(l); });
    collapseDownstreamCards();
...
nextToCard2Btn.addEventListener('click', function() { openCard2(); });
...
function updateApiKeyLabel(displayName) {
    ...
    if (hint && pricingData) {
        ...
        var docsLink = prov.apiTokenDocs
            ? ' <a href="' + prov.apiTokenDocs + '" target="_blank" rel="noopener">See how to create one &rarr;</a>'
            : '';
        hint.innerHTML = 'Create a token with permissions to manage servers/droplets/instances.'
            + docsLink;
    }
}
...
function renderAcks() {
    var acks = [
        { id:'ack-pricing',        text:"Prices shown are estimates. I'll check current pricing before deploying." },
        { id:'ack-responsibility', text:"I'm responsible for all charges on my account. This tool just generates config files." }
    ];
    var html = '';
    for (var i = 0; i < acks.length; i++) {
        var a = acks[i];
        html += '<label class="ack-item">'
            + '<input type="checkbox" class="billing-ack-cb" id="' + a.id + '">'
            + '<span>' + a.text + '</span></label>';
    }
...
function checkAcks() {
    ...
    var done = all.length > 0 && all.every(function(cb) { return cb.checked; });
...
el('hasTailscaleBtn').addEventListener('click', function() {
    ...
    el('credentialsForm').scrollIntoView({ behavior:'smooth', block:'nearest' });
});
...
tailscaleKeyInput.addEventListener('input', function() {
    ...
    tsKeyWarning.style.display = (v.length > 0 && v.indexOf('tskey-') !== 0) ? 'block' : 'none';
});

nextToCard4Btn.addEventListener('click', function() {
    if (!tailscaleKeyInput.value.trim()) { tailscaleKeyInput.focus(); return; }
    if (!providerApiKeyInput.value.trim()) { providerApiKeyInput.focus(); return; }
    state.skippedCredentials = false;
    renderSummary();
    revealCard(card4);
...
function renderSummary() {
    ...
    for (var i = 0; i < prov.plans.length; i++) {
        if (prov.plans[i].code === state.selectedPlan.code) { plan = prov.plans[i]; break; }
    }
    var region = null;
    for (var j = 0; j < prov.regions.length; j++) {
        if (prov.regions[j].code === state.selectedRegion) { region = prov.regions[j]; break; }
    }
    ...
    summaryGrid.innerHTML =
        '<div class="summary-item"><span class="summary-label">Provider</span><span class="summary-value">' + prov.displayName + '</span></div>'
        + '<div class="summary-item"><span class="summary-label">Plan</span><span class="summary-value">' + plan.name + ' &middot; ' + plan.vcpu + ' vCPU / ' + plan.ram + ' / ' + plan.disk + '</span></div>'
        + '<div class="summary-item"><span class="summary-label">Monthly Cost</span><span class="summary-value summary-price">$' + plan.price.toFixed(2) + '/mo</span></div>'
        + '<div class="summary-item"><span class="summary-label">Region</span><span class="summary-value">' + (region ? region.name : state.selectedRegion) + '</span></div>'
        + '<div class="summary-item"><span class="summary-label">Tailscale Key</span><span class="summary-value summary-secret">' + tsDots + '</span></div>'
        + '<div class="summary-item"><span class="summary-label">' + prov.displayName + ' Token</span><span class="summary-value summary-secret">' + apiDots + '</span></div>';
}
...
downloadBtn.addEventListener('click', function() {
    generateFiles(
        state.selectedProvider, state.selectedPlan, state.selectedRegion,
        tailscaleKeyInput.value.trim(), providerApiKeyInput.value.trim()
    ).then(function(files) {
...
function generateFiles(provider, planRef, region, tsKey, apiKey) {
    ...
    for (var i = 0; i < prov.plans.length; i++) {
        if (prov.plans[i].code === planRef.code) { plan = prov.plans[i]; break; }
    }
    return Promise.all([
        fetch('templates/' + provider + '/main.tf.template').then(function(r) { return r.text(); }),
        fetch('templates/' + provider + '/variables.tf.template').then(function(r) { return r.text(); }),
        fetch('templates/' + provider + '/cloud-init.yaml.template').then(function(r) { return r.text(); })
    ]).then(function(tpls) {
        var mainTf    = tpls[0].replace(/\{\{PROVIDER_CONFIG\}\}/g,  buildProviderConfig(provider, plan));
        var varsTf    = tpls[1].replace(/\{\{VARIABLES_CONFIG\}\}/g, buildVariableConfig(provider));
        var cloudInit = tpls[2].replace(/\{\{CLOUD_INIT_CONFIG\}\}/g, buildCloudInit(tsKey));
        var actualTsKey  = tsKey  || 'PASTE_YOUR_TAILSCALE_AUTH_KEY_HERE';
        var actualApiKey = apiKey || 'PASTE_YOUR_PROVIDER_API_TOKEN_HERE';
        var tfvars    = '# Auto-generated -- keep secret! NEVER commit to version control.\n\n'
            + 'provider_api_token = "' + actualApiKey + '"\n'
            + 'tailscale_auth_key = "' + actualTsKey  + '"\n'
            + 'target_region      = "' + region + '"\n'
            + 'vm_size            = "' + plan.code + '"\n';
        var tfvarsEx  = '# Copy to terraform.tfvars and fill in real values\n# NEVER commit terraform.tfvars!\n\n'
            + 'provider_api_token = "your-' + provider + '-api-token-here"\n'
            + 'tailscale_auth_key = "tskey-auth-xxxxxxxxxxxx"\n'
            + 'target_region      = "' + region + '"\n'
            + 'vm_size            = "' + plan.code + '"\n';
        return {
            'main.tf': mainTf, 'variables.tf': varsTf, 'cloud-init.yaml': cloudInit,
            'terraform.tfvars': tfvars, 'terraform.tfvars.example': tfvarsEx,
...
function buildDeployScriptPS(provider) {
    return '# deploy.ps1 -- Tunnel Digger deployment helper\n'
        + '# Usage: .\\deploy.ps1\n\n'
...
function buildReadme(prov, plan, region) {
    ...
    return '# Tailscale Exit Node -- ' + prov.displayName + '\nGenerated: ' + date + '\n\n'
        + '## Quick Start\n\n```bash\n./deploy.sh   # or .\\deploy.ps1 on Windows\n```\n\n'
        + 'Approve exit node: https://login.tailscale.com/admin/machines\n\n'
        + '## Cleanup\n```bash\nterraform destroy\n```\n\n'
        + 'Estimated cost: ~$' + plan.price.toFixed(2) + '/mo\n'
        + 'Docs: ' + prov.website + '\n';
}
...
function showDeploymentInstructions(provider) {
    ...
    var tailscaleNote = '<div class="info-box" style="margin-top:16px;">'
    ...
    var billingNote = '<div class="info-box" style="margin-top:16px;">'
        ...
        + '<a href="' + prov.billingPage + '" target="_blank" rel="noopener" class="billing-link" style="margin-top:10px;">'
        + 'Open ' + prov.displayName + ' Billing Alerts &rarr;</a></div>';

    var linux = [
        { label:'Unzip the package',         cmd:'unzip tailscale-exit-node-' + provider + '.zip -d tailscale-exit-node' },
    ...
    var win = [
        { label:'Unzip (PowerShell)',         cmd:'Expand-Archive tailscale-exit-node-' + provider + '.zip -DestinationPath tailscale-exit-node' },
        { label:'Enter the folder',          cmd:'cd tailscale-exit-node' },
...
function renderCmdBlocks(containerId, steps, platform) {
    ...
    for (var i = 0; i < steps.length; i++) {
        var s = steps[i];
        html += '<div class="cmd-block' + (s.warn ? ' cmd-block-warn' : '') + '" id="cmd-' + platform + '-' + i + '">'
            + '<div class="cmd-label">' + s.label + '</div>'
            + '<div class="cmd-row"><code class="cmd-code">' + escHtml(s.cmd) + '</code>'
            + '<button class="cmd-copy-btn" onclick="copyCmd(\'' + platform + '\',' + i + ',this)">Copy</button>'
            + '</div></div>';
    }
...
function copyCmd(platform, i, btn) {
    ...
    navigator.clipboard.writeText(block.querySelector('.cmd-code').textContent).then(function() {
...
function showTab(platform) {
    ...
    el('windowsCommands').style.display = platform === 'windows' ? 'block' : 'none';
}
